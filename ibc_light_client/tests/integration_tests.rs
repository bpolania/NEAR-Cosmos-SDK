...
const IBC_LIGHT_CLIENT_WASM: &[u8] = include_bytes!("../target/near/ibc_light_client.wasm");

/// Helper function to deploy the IBC Light Client contract
async fn deploy_light_client_contract(worker: &Worker<near_workspaces::network::Sandbox>) -> Result<Contract> {
    let contract = worker.dev_deploy(IBC_LIGHT_CLIENT_WASM).await?;
...
mod light_client_tests {
    ...
    async fn test_create_client() -> Result<()> {
        ...
        let contract = deploy_light_client_contract(&worker).await?;
